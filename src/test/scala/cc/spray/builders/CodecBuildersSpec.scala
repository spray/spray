...
class CodecBuildersSpec extends Specification with SprayTest with ServiceBuilder {
  ...
  "the NoEncoding decoder" should {
    "decode the request content if it has encoding 'identidy'" in {
      test(HttpRequest(headers = List(`Content-Encoding`(Encodings.identity)), content = Some(HttpContent(`text/plain`, "yes")))) { 
    ...
    "reject requests with content encoded with 'deflate'" in {
      test(HttpRequest(headers = List(`Content-Encoding`(Encodings.deflate)), content = Some(HttpContent(`text/plain`, "yes")))) { 
        decodeRequest(NoEncoding) { completeOk }
      }.rejections mustEqual Set(UnsupportedRequestEncodingRejection(Encodings.identity))
    }
  ...
  "the Gzip decoder" should {
    "decode the request content if it has encoding 'gzip'" in {
      ...
      test(HttpRequest(headers = List(`Content-Encoding`(Encodings.gzip)),
    ...
    "reject requests with content encoded with 'deflate'" in {
      test(HttpRequest(headers = List(`Content-Encoding`(Encodings.deflate)),
        content = Some(HttpContent(`text/plain`, "Hello")))) { 
        decodeRequest(Gzip) { completeOk }
      }.rejections mustEqual Set(UnsupportedRequestEncodingRejection(Encodings.gzip))
    }
  ...
  "the Gzip encoder" should {
    ...
    "reject the request if the client does not accept GZIP encoding" in {
      test(HttpRequest(headers = List(`Accept-Encoding`(Encodings.identity)))) { 
        encodeResponse(Gzip) { _.complete("Yeah!") }
      }.rejections mustEqual Set(UnacceptedResponseEncodingRejection(Encodings.gzip))
    }
