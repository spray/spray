  ...
  def event(ev: CometEvent) {
    ...
    ev.getEventType match {
      ...
      case CometEvent.EventType.ERROR => ev.getEventSubType match {
        ...
        case CometEvent.EventSubType.CLIENT_DISCONNECT => logWarn("Client disconnected for %s", req)
        case err => logError("Unspecified Error during async processing of %s:\n%s", req, err)
      }
