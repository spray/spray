      ...
      val headers = request.header[HttpHeaders.Host] match {
        case Some(reqHostHeader) ⇒
          if (authority != Authority(Host(reqHostHeader.host), reqHostHeader.port).normalizedForHttp())
            log.warning("sending request with header '" + reqHostHeader + "' to a proxied connection to " + authority)
          request.headers
        case None ⇒
      ...
      val effectiveUri =
        if (request.uri.isRelative)
          ...
          if (authority != request.uri.authority.normalizedForHttp())
            log.warning("sending request with absolute URI '" + request.uri + "' to a proxied connection to " + authority)
          request.uri
        }
