...
object ProxySettings extends SettingsCompanion[Map[String, ProxySettings]]("spray.can.client.proxy") {
  ...
  def fromProperties(properties: Map[String, String], scheme: String): Option[ProxySettings] = {
    val proxyHost = properties.get(scheme + ".proxyHost")
    val proxyPort = properties.get(scheme + ".proxyPort")
    val nonProxyHosts = properties.get(scheme + ".nonProxyHosts")
    proxyHost map (apply(
      _,
    ...
    def proxySettings(scheme: String) = c.getValue(scheme).valueType() match {
      case ConfigValueType.STRING ⇒
        c.getString(scheme) match {
          ...
          case "none"    ⇒ None
          case unknown   ⇒ throw new IllegalArgumentException("illegal value for proxy." + scheme + ": '" + unknown + '\'')
        }
      case _ ⇒
