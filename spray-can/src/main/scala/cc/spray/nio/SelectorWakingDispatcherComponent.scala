...
trait SelectorWakingDispatcherComponent {
   ...
   */
  class SelectorWakingDispatcher(threadName: String) extends ExecutorBasedEventDrivenDispatcher(
    _name = threadName,
    throughput = -1,
    ...
    override def dispatch(invocation: MessageInvocation) {
      ...
      if (invocation.message != IOActor.Select) selector.wakeup()
    }
