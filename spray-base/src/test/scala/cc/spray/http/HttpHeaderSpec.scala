...
class HttpHeaderSpec extends Specification {
  ...
  "Header 'Accept-Charset'" should {
    ...
    "be parsed correctly from various examples" in {
      ...
      HttpHeader("Accept-Charset", "pipapo; q= 1.0, utf-8") mustEqual `Accept-Charset`(new CustomHttpCharset("pipapo"), `UTF-8`)
    }
  }
  ...
  "Header 'Accept-Encoding'" should {
    ...
    "be parsed correctly from various examples" in {
      HttpHeader("Accept-Encoding", "compress, gzip, fancy") mustEqual `Accept-Encoding`(compress, gzip, new CustomHttpEncoding("fancy"))
  ...
  "Header 'Content-Disposition'" should {
    "be parsed correctly from various examples" in {
      HttpHeader("Content-Disposition", "form-data") mustEqual `Content-Disposition`("form-data", Map.empty)
      HttpHeader("Content-Disposition", "attachment; name=\"field1\"; filename=\"file.txt\"") mustEqual
  ...
  "Header 'Content-Encoding'" should {
    ...
    "be parsed correctly from various examples" in {
      ...
      HttpHeader("Content-Encoding", "pipapo") mustEqual `Content-Encoding`(new CustomHttpEncoding("pipapo"))
    }
  }
