...
class HttpHeaderSpec extends Specification {
  ...
  implicit def TestLine(line: String): TestLine = new TestLine(line)
  class TestLine(line: String) {
  ...
  implicit def TestHeader(header: HttpHeader): TestHeader = new TestHeader(header)
  class TestHeader(header: HttpHeader) extends TestExample {
    def apply(line: String) = {
      ...
      if (h.isRight && h.right.get.isInstanceOf[RawHeader] && !header.isInstanceOf[RawHeader])
        failure("`" + line + "` was not parsed into a ModeledHeader")
  ...
  implicit def TestError(expectedError: ErrorInfo): TestError = new TestError(expectedError)
  class TestError(expectedError: ErrorInfo) extends TestExample {
    def apply(line: String) = {
      val Array(name, value) = line.split(": ", 2)
