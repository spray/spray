...
trait TransformerPipelineSupport {
  ...
  implicit def pimpWithTransformation[A](value: A) = new WithTransformation(value)
  class WithTransformation[A](value: A) {
    def ~>[B](f: A ⇒ B): B = f(value)
  }

  implicit def pimpWithTransformationConcatenation[A, B](f: A ⇒ B) = new WithTransformerConcatenation(f)
  class WithTransformerConcatenation[A, B](f: A ⇒ B) extends (A ⇒ B) {
