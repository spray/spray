_spray-can_ is a lightweight, low-overhead, high-performance, fully asynchronous HTTP 1.0/1.1 server and client library
implemented entirely in [Scala] and [Akka].
...
* No dependencies except for JavaSE, [Scala] and [Akka][] (actors module).
...
next layer in the application stack and not implemented by _spray-can_ itself. Apart from general focus this design
...
8. Start hacking on the sources in `server-example/src/main/scala/cc/spray/can/example/` and/or
...
The _spray-can_ HTTP server is really easy to use. All you need to do is to start a new [HttpServer] actor as well as
...
`HttpHeader("Connection", "close")`. To unconditionally force a keep-alive you can explicitly set a
...
a whole sequence of requests in a row without first waiting for responses. The _spray-can_ `HttpServer` will dispatch
these pipelined requests to your service actor just as any other. However, since in many asynchronous applications
...
sending of very large (even potentially infinite) HTTP requests or responses. Normally the client or server sending an
...
stopping the actor your custom function created as well as dispatching [MessageChunk], [ChunkedRequestEnd] and
potential [HttpClientException] messages to it. If you do not supply a custom `streamActorCreator` _spray-can_ uses
a [BufferingRequestStreamActor] for incoming chunked requests to transparently buffer and assemble regular
[HttpResponse] instances before dispatching them to the regular service actor.
...
returns a [ChunkedResponder] that allows for the sending of the individual message chunks as well as response
finalization.
...
The _spray-can_ `HttpClient` is the natural dual of the `HttpServer`. It shares all core features as well as the basic
...
Additionally the `HttpClient` will close idle connections if the configured `idleTimeout` is non-zero.
