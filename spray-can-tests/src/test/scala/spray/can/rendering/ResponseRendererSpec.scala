    ...
    "properly render" in {
      ...
      "a response with status 400, a few headers and a body" in {
        Context(
          HttpResponse(
            ...
            protocol = `HTTP/1.0`)) must beRenderedTo(
      ...
      "a response to a HEAD request" in {
        Context(
          response = HttpResponse(
            headers = List(RawHeader("Age", "30"), RawHeader("Connection", "Keep-Alive")),
            entity = "Small f*ck up overhere!"),
          requestMethod = HttpMethods.HEAD) must beRenderedTo(
            content = """|HTTP/1.1 200 OK
                       |Server: spray-can/1.0.0
                       |Date: Thu, 25 Aug 2011 09:10:29 GMT
                       |Content-Type: text/plain
      ...
      "a non-keepalive HTTP/1.0 message" in {
        Context(
          HttpResponse(
            ...
            protocol = `HTTP/1.0`,
            entity = "Small f*ck up overhere!")) must beRenderedTo(
      ...
      "a chunked response without body" in {
        Context(
          ...
          requestConnectionHeader = Some("close")) must beRenderedTo(
            content = """|HTTP/1.1 200 OK
                       |Server: spray-can/1.0.0
                       |Date: Thu, 25 Aug 2011 09:10:29 GMT
                       |Age: 30
      ...
      "a response chunk" in {
        Context(
          MessageChunk(
            ...
            List(ChunkExtension("key", "value"), ChunkExtension("another", "tl;dr")))) must beRenderedTo(
      ...
      "a final response chunk" in {
        Context(
          ChunkedMessageEnd(Nil, List(RawHeader("Age", "30"), RawHeader("Cache-Control", "public")))) must beRenderedTo(
            content = """|0
                       |Age: 30
                       |Cache-Control: public
                       |
      ...
      "a chunkless chunked response without body" in {
        Context(
          response = ChunkedResponseStart(HttpResponse(200, headers = List(RawHeader("Age", "30")))),
          chunkless = true) must beRenderedTo(
            content = """|HTTP/1.1 200 OK
                       |Server: spray-can/1.0.0
                       |Date: Thu, 25 Aug 2011 09:10:29 GMT
                       |Age: 30
          ...
          `HTTP/1.0` ! Some("close") ! Some("Keep-Alive") ! Some("Keep-Alive") ! false |> {
            ...
            (reqProto, reqCH, resCH, renCH, close) ⇒
              Context(
                ...
                requestConnectionHeader = reqCH) must beRenderedTo(
                  content = "HTTP/1.1 200 OK\n" +
                    "Server: spray-can/1.0.0\n" +
                    "Date: Thu, 25 Aug 2011 09:10:29 GMT\n" +
